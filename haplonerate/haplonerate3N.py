...
def delete_extra(seq1,seq2,seq3,ref,phaseBlock):
#    if seq1.seq == seq2.seq:
#        return [SeqRecord(ref.seq,id=ref.id,description='')]
#    else:
    ...
    haplo3 = SeqRecord(seq3.seq[phaseBlock[0]:phaseBlock[1]],id="{}_h3".format(seq1.id),description='')
    return [haplo1,haplo2,haplo3]

def main():
    parser = argparse.ArgumentParser(description=helptext,formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument("gtf",help="gtf file annotating the positions of phase blocks for each gene")
    parser.add_argument("haplotype_files",help="Three FASTA files containing sequences for one or more genes",nargs="+")
    ...
    if len(args.haplotype_files) != 3:
        print("Please supply exactly three haplotype FASTA files!\n")
        sys.exit(1)
    ...
    haplotype3_dict = SeqIO.to_dict(SeqIO.parse(args.haplotype_files[2],'fasta'))
        ...
        if gene in gtf_dict:
            if gene in ref_dict:
                ...
                if args.edit == 'ref':
                    ...
                    seqs_to_write += delete_extra(haplotype1_dict[gene],haplotype2_dict[gene],haplotype3_dict[gene],ref_dict[gene],phaseBlock)
        else:
            if gene in ref_dict:
